defmodule DatabaseConn.Getuser do
  ...
  defp getter(id) do
    case Project.Repo.one(from u in Project.User, where: u.globaluserid == ^id) do
      nil -> {:error, "user with ID #{id} not found" }
      user -> {:ok, user}
    end
  end
  @spec checkuser(Ecto.UUID.t()) :: {:ok, nil} | {:error,Project.User.t()}
  def checkuser(userid) do
    case Project.Repo.one(from u in Project.User, where: u.globaluserid == ^userid) do
      nil -> {:ok,nil}
      user -> {:error, user}
    end
  end
