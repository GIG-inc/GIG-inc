defmodule Comms.Receiver do
  ...
  require Logger
  alias Protoservice.{CapitalRaiseResp, Tra}
  alias Internalservice.TransferResp
  ...
  def transfer(request, _stream) do
    ...
    case Registry.lookup(Project.Registry, "transfer-#{userid}") do
      ...
      [] ->
        case DynamicSupervisor.start_child(Project.Dynamicsupervisor, {Transfer, name: "transfer-#{userid}"}, restart: :transient) do
          {:ok, pid} ->
            GenServer.call(pid, {:transfer, request})
          {:error,message} ->
            IO.puts("there was an issue attempting a transfer")
            %TransferResp{
              # TODO: finish this
            }
        end
  ...
  def createaccount(request, _stream) do
    case Registry.lookup(Project.Registry, "account-#{request.globaluserid}") do
      [{pid, _}] ->
      ...
      [] ->
        case DynamicSupervisor.start_child(
               ...
               {Createuser, name: "account-#{request.globaluserid}"}
             ) do
          {:ok, pid} ->
            GenServer.call(pid, {:createuser, request})
